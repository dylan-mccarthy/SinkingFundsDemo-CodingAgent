	...
	const transactionTypes = [
		{
			value: 'EXPENSE',
			label: 'Expense',
			description: 'Money spent from this fund',
			color: 'text-red-600'
		},
		{
			value: 'INCOME',
			label: 'Income',
			description: 'Money added to this fund',
		...
		{:else if funds.length === 0}
			<div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
				<p class="text-yellow-800 mb-4">
					You need to create at least one fund before adding transactions.
				</p>
				<button
					...
					<div class="mb-6">
						...
						{#if selectedType && selectedFund && amount}
							<p class="text-sm mt-1 {selectedType.color}">
								{selectedType.value === 'EXPENSE' ? 'Will reduce' : 'Will increase'} fund balance by
						...
						<div class="bg-gray-50 rounded-lg p-4 mb-6">
							...
							<div class="space-y-1 text-sm">
								...
								<p>
									...
									<span class={selectedType.color}>{selectedType.label}</span>
								</p>
								<p>
									<span class="text-gray-600">Amount:</span> ${parseFloat(amount || '0').toFixed(2)}
								</p>
								<p>
									<span class="text-gray-600">New Balance:</span>
									${(
										(selectedFund.balance +
											(selectedType.value === 'EXPENSE' ? -1 : 1) *
												parseFloat(amount || '0') *
												100) /
										100
									).toFixed(2)}
