...
function calculateFundLevel(balanceCents: number): { level: number; progress: number; nextLevelAt: number } {
...
function calculateSavingsStreak(transactions: any[]): { currentStreak: number; longestStreak: number } {
...
function calculateBadges(funds: any[], transactions: any[]): any[] {
	...
	if (totalBalance >= 100000) badges.push({ id: 'first_1000', name: 'First $1,000', emoji: '🎯', description: 'Saved your first $1,000' });
	if (totalBalance >= 500000) badges.push({ id: 'halfway_5000', name: 'Halfway Hero', emoji: '🏃', description: 'Reached $5,000 in total savings' });
	if (totalBalance >= 1000000) badges.push({ id: 'master_10000', name: 'Savings Master', emoji: '👑', description: 'Achieved $10,000 in total savings' });
	
	// Fund goal achievements
	const goalsReached = funds.filter(f => f.targetCents > 0 && f.balance >= f.targetCents).length;
	if (goalsReached >= 1) badges.push({ id: 'goal_crusher', name: 'Goal Crusher', emoji: '🎉', description: 'Reached your first fund goal' });
	if (goalsReached >= 3) badges.push({ id: 'triple_threat', name: 'Triple Threat', emoji: '🔥', description: 'Reached 3 fund goals' });
	
	// Transaction-based badges
	if (transactions.length >= 10) badges.push({ id: 'active_tracker', name: 'Active Tracker', emoji: '📊', description: 'Recorded 10+ transactions' });
	if (transactions.length >= 50) badges.push({ id: 'transaction_hero', name: 'Transaction Hero', emoji: '⚡', description: 'Recorded 50+ transactions' });
	
	// Streak badges
	const streakData = calculateSavingsStreak(transactions);
	if (streakData.currentStreak >= 3) badges.push({ id: 'streak_starter', name: 'Streak Starter', emoji: '🔥', description: '3-month savings streak' });
	if (streakData.currentStreak >= 6) badges.push({ id: 'streak_master', name: 'Streak Master', emoji: '🌟', description: '6-month savings streak' });
